...
fn main() {
    ...
    let report = |label: &str, r: PsResult<SurfIntersection>| match r {
        Ok(si) => {
            let types: Vec<String> = si
                .curves
                .iter()
                .map(|c| format!("{:?}", c.curve.curve_type()))
                .collect();
            println!(
                "  {:52} -> {} pts, {} curves {:?}",
                label,
    ...
    report(
        "sphere(r=8) ∩ plane(z=3)   [expect Circle]",
        sph.intersect(&pl_z3),
    );
    let cyl_z = Surf::cylinder(Axis2::new(o, z, x), 5.0).unwrap();
    let pl_z0 = Surf::plane(Axis2::new(o, z, x)).unwrap();
    report(
        "cylinder(Z,r=5) ∩ plane(z=0) [expect Circle]",
        cyl_z.intersect(&pl_z0),
    );
    let nrm = {
        let l = (2.0_f64).sqrt();
        Vec3::new(1.0 / l, 0.0, 1.0 / l)
    };
    let pl_tilt = Surf::plane(Axis2::new(o, nrm, y)).unwrap();
    report(
        "cyl(Z,r=5) ∩ plane(45deg,normalized) [expect Ellipse]",
        cyl_z.intersect(&pl_tilt),
    );
    let nrm2 = {
        let l = (1.0_f64 + 16.0).sqrt();
        Vec3::new(1.0 / l, 0.0, 4.0 / l)
    };
    let pl_t2 = Surf::plane(Axis2::new(o, nrm2, y)).unwrap();
    report(
        "cyl(Z,r=5) ∩ plane(shallow 14deg) [expect Ellipse]",
        cyl_z.intersect(&pl_t2),
    );
    let sphA = Surf::sphere(Axis2::new(o, z, x), 8.0).unwrap();
    let sphB = Surf::sphere(Axis2::new(Vec3::new(10.0, 0.0, 0.0), z, x), 6.0).unwrap();
    report(
        "sphere(o,r=8) ∩ sphere((10,0,0),r=6) [expect Circle]",
        sphA.intersect(&sphB),
    );
    // quartic rows
    let cyl_x = Surf::cylinder(Axis2::new(o, x, y), 5.0).unwrap();
    report(
        "cyl(Z,r=5) ∩ cyl(X,r=5)    [Steinmetz, expect Icurve/quartic]",
        cyl_z.intersect(&cyl_x),
    );
    let cyl_x4 = Surf::cylinder(Axis2::new(o, x, y), 4.0).unwrap();
    report(
        "cyl(Z,r=5) ∩ cyl(X,r=4)    [unequal, expect Icurve/quartic]",
        cyl_z.intersect(&cyl_x4),
    );
    let sph6 = Surf::sphere(Axis2::new(Vec3::new(0.0, 0.0, 4.0), z, x), 6.0).unwrap();
    report(
        "cyl(Z,r=5) ∩ sphere((0,0,4),r=6) [expect Icurve/quartic]",
        cyl_z.intersect(&sph6),
    );
}
