...
fn main() {
    ...
    println!(
        "orig vol={} faces={} edges={}",
        b.volume().unwrap(),
        b.faces().unwrap().len(),
        b.edges().unwrap().len()
    );
    match parasolid::fileio::transmit(&[b], "rt") {
        Ok(()) => println!("transmit OK"),
        Err(e) => {
            println!("transmit err: {e}");
            return;
        }
    }
    match parasolid::fileio::receive("rt") {
        Ok(bs) => {
            println!("received {} bodies", bs.len());
            for rb in &bs {
                println!(
                    "  vol={} faces={} edges={} type={:?}",
                    rb.volume().unwrap(),
                    rb.faces().unwrap().len(),
                    rb.edges().unwrap().len(),
                    rb.body_type().unwrap()
