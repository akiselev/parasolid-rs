...
fn main() {
    ...
    for &ap in &[1e-11_f64, 1e-9, 1e-13, 1e-11] {
        unsafe {
            parasolid_sys::PK_SESSION_set_angle_precision(ap);
        }
        let mut got = 0.0;
        unsafe {
            parasolid_sys::PK_SESSION_ask_angle_precision(&mut got);
        }
        let (lo, hi) = threshold();
        println!(
            "angle_precision set={:.0e} ask={:.3e} -> plane-tilt line-appears at theta ~ {:.3e}..{:.3e}",
            ap, got, lo, hi
        );
    }
    let _ = s;
